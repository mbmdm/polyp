...
//BufferResource createBuffer(const Device& device, vk::BufferUsageFlags usage, uint32_t size, vk::MemoryPropertyFlags props) {
//    auto vk = device->vk();
//    auto gpu = device->gpu();
...
auto createVertexBuffer(const Device& device, const PhysicalDevice& gpu, const Queue& queue, const CommandPool& mCmdPool)
{
    void* mapped = nullptr;
    ...
    CommandBuffer cmd = std::move(cmds[0]);
