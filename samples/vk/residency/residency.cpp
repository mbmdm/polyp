...
/*
Links:
https://habr.com/ru/articles/515630/                                                                       - Sparse residency textures in Vulkan [rus]
https://asawicki.info/news_1698_vulkan_sparse_binding_-_a_quick_overview                                   - Vulkan sparse binding - a quick overview [en]
https://github.com/SaschaWillems/vulkan_slim/blob/master/texturesparseresidency/texturesparseresidency.cpp - Sascha Willems Vulkan samples
*/

namespace {

constexpr uint32_t gTexWidth = 512;
constexpr uint32_t gTextHeight = 512;
constexpr uint32_t gTextPixelSize = 4;        // 4 components per texel - RGBA
constexpr uint32_t gNumTextures = 1024 * 8;   // ~8GB of VRAM. Each texture is 512 * 512 * 4 = 1Mb size
constexpr uint32_t gNumTexProducers = 4;      // number of threads whitch creates textures of gNumTextures in parralel during rendering
...
auto createVertexBuffer(Device::ConstPtr device)
{
    ...
    std::vector<uint32_t> indexBuffer{ 2, 1, 2 };
